...
int CMesh::CheckRayIntersection(XMFLOAT3& xmvPickRayOrigin, XMFLOAT3& xmvPickRayDirection, float* pfNearHitDistance)
{
	...
	if (bIntersected)
	{
		...
		{
			XMVECTOR v0 = XMLoadFloat3(&m_pxmf3Positions[(m_pnSubSetIndices) ? m_pnSubSetIndices[(i * nOffset) + 0] : ((i * nOffset) + 0)]);
			XMVECTOR v1 = XMLoadFloat3(&m_pxmf3Positions[(m_pnSubSetIndices) ? m_pnSubSetIndices[(i * nOffset) + 1] : ((i * nOffset) + 1)]);
			XMVECTOR v2 = XMLoadFloat3(&m_pxmf3Positions[(m_pnSubSetIndices) ? m_pnSubSetIndices[(i * nOffset) + 2] : ((i * nOffset) + 2)]);
