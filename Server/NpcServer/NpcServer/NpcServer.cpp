...
void SERVER::do_send(void* packet)
{
	...
	if (ret != 0 && GetLastError() != WSA_IO_PENDING) {
		...
		if (GetLastError() == WSAECONNRESET) {
			closesocket(g_logicservers[a_lgcsvr_num].sock);
			cout << "[Failover] Logic Server[" << a_lgcsvr_num << "]의 다운이 감지되었습니다." << endl;	// server message
...
void initNpc() {
	for (int i{}; i < HelicopterNum; i++) {
		...
		uniform_real_distribution<float>SpdSet(5.0f, 12.0f);
