...
enum PacketID {
	...
	, LBYC_UNIQUE_ID ,LBYC_MATCH_FAIL, LBYC_ROOM_JOIN, LBYC_ADD_ROOM, LBYC_ROOM_USERCOUNT, LBYC_ROOM_NEW_MEMBER, LBYC_ROOM_LEFT_MEMBER
	, LBYC_LOBBY_CLEAR, LBYC_MEMBER_STATE, LBYC_ROLE_CHANGE, LBYC_GAME_START, LBYC_GAME_EXIT, LBYC_POPUP
	, LBYLBY_MEMBER_REPLICA, LBYLBY_ROOM_REPLICA, LBYLBY_ROOM_ERASE
	...
	, SC_DAMAGED, SC_HEALING, SC_RESPAWN, SC_HEALPACK, SC_ATTACK, SC_RELOAD, SC_CHANGE_SCENE, SC_OBJECT_STATE, SC_TIMEOUT, SC_BULLET_COLLIDE_POS, SC_MISSION, SC_MISSION_COMPLETE
	, SC_TIME_TICKING, SC_CHAT, SC_MAP_OBJINFO, SC_PING_RETURN, SC_ACTIVE_DOWN, SC_STANDBY_RUN, SC_RESET_GAME
	, SS_CONNECT, SS_HEARTBEAT, SS_USER_REPLICA, SS_GAME_REPLICA, SS_NPC_REPLICA
...
// 2. 로비서버 -> 클라
struct LBYC_UNIQUE_ID_PACKET {
	unsigned char size;
	char type;
	short unique_id;	// 딱 그 클라만 갖고 있는 고유 ID
};
...
// 3. 로비서버 간 HA
struct LBYLBY_MEMBER_REPLICA_PACKET {
	unsigned char size;
	char type;
	short id;
	char name[NAME_SIZE];
...
struct LBYLBY_ROOM_REPLICA_PACKET {
	unsigned char size;
	char type;
	short users[MAX_USER];
	short room_id;
	char room_name[ROOM_NAME_SIZE];
	char room_state;
	short user_count;
	short total_room_count;
};

struct LBYLBY_ROOM_ERASE_PACKET {
	unsigned char size;
	char type;
	short room_id;
};
...
// 4) UI 관련 패킷
#define STAGE1_TIMELIMIT 366 // 스테이지1 제한시간 (단위: sec)
