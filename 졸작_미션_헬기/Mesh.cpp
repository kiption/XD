...
// 높이 맵 이미지 : raw 파일을 로드하여 어떻게 읽을 건지
	...
	XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void* pContext) : CTerrainMesh(pd3dDevice, pd3dCommandList)
{
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			pVertices[i] = CNormalVertex(xmf3Position, pHeightMapImage->GetHeightMapNormal(x,z));
...
XMFLOAT4 CHeightMapGridMesh::OnGetColor(int x, int z, void* pContext)
{
	...
	//조명의 색상(세기, 밝기)이다. 
	XMFLOAT4 xmf4IncidentLightColor(0.3f, 0.5f, 0.3f, 0.5f);
...
CCubeMeshDiffused::CCubeMeshDiffused(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight, float fDepth)
{
	...
	oobb = BoundingOrientedBox(XMFLOAT3(0.0,0.0,0.0), XMFLOAT3(15.0, 15.0, 15.0), XMFLOAT4(0.0, 0.0, 0.0, 1.0));
}
