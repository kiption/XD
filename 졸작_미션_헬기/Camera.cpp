...
CCamera::CCamera(CCamera *pCamera)
{
	...
	{
		m_xmf4x4View = Matrix4x4::Identity(); //뷰행렬
		m_xmf4x4Projection = Matrix4x4::Identity(); //투영행렬
		m_d3dViewport = { 0, 0, FRAME_BUFFER_WIDTH , FRAME_BUFFER_HEIGHT, 0.0f, 1.0f }; // 뷰포트 == 시저사각형
		m_d3dScissorRect = { 0, 0, FRAME_BUFFER_WIDTH , FRAME_BUFFER_HEIGHT };
		m_xmf3Position = XMFLOAT3(0.0f, 0.0f, 0.0f); // 카메라 위치
		m_xmf3Right = XMFLOAT3(1.0f, 0.0f, 0.0f); //벡터들의 값
		m_xmf3Look = XMFLOAT3(0.0f, 0.0f, 1.0f);
		m_xmf3Up = XMFLOAT3(0.0f, 1.0f, 0.0f);
		m_fPitch = 0.0f; // 카메라 회전 값
		m_fRoll = 0.0f;
		m_fYaw = 0.0f;
		m_xmf3Offset = XMFLOAT3(0.0f, 0.0f, 0.0f); // 카메라와 플레이어 사이 거리
		m_fTimeLag = 0.0f; // 회전 격차
		m_xmf3LookAtWorld = XMFLOAT3(0.0f, 0.0f, 0.0f);
		m_nMode = 0x00; // 인칭 모드
		m_pPlayer = NULL;
	}
...
{
	fPitch = m_pPlayer->GetPitch();
	fYaw = m_pPlayer->GetYaw();
	fRoll = m_pPlayer->GetRoll();
