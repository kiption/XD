...
void CObjectsShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, void *pContext)
{
    ...
    for (int h = 0; h < nFirstPassColumnSize; h++)
    {
        for (int i = 0; i < floor(float(m_nObjects) / float(nColumnSize)); i++)
        {
			...
			float fHeight = pTerrain->GetHeight(390.0f, 670.0f)+100.0f;
			XMFLOAT3 xmf3Position = RandomPositionInSphere(XMFLOAT3(390.0f, fHeight + 90.0f, 670.0f), Random(180.0f, 250.0f), h - int(floor(nColumnSize / 2.0f)), nColumnSpace);
			...
			m_ppObjects[nObjects]->Rotate(0.0f, 0.0f, 0.0f);
    ...
    if (nFirstPassColumnSize != nColumnSize)
    {
        for (int i = 0; i < m_nObjects - int(floor(float(m_nObjects) / float(nColumnSize)) * nFirstPassColumnSize); i++)
        {
			...
			m_ppObjects[nObjects]->SetPosition(RandomPositionInSphere(XMFLOAT3(0.0f, 0.0f, 0.0f), Random(90.0f, 180.0f), nColumnSize - int(floor(nColumnSize / 2.0f)), nColumnSpace));
...
void CBulletMotionShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
{
	...
	ppSpriteMaterials[0] = new CMaterial(1);
	...
	{
		CreateShaderResourceViews(pd3dDevice, ppSpriteTextures[i], 0, 16);
	}
	...
	for (int j = 0; j < m_nObjects; j++)
	{
		...
		pExplodeObject->SetMesh(pSpriteMesh[j]);
		pExplodeObject->SetMaterial(ppSpriteMaterials[j]);
...
void CBulletMotionShader::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera, int nPipelineState)
{
	...
	if (m_bBulletActive == true)
	{
		...
		for (int i = 0; i < 1; i++)
		{
			m_ppObjects[i]->SetLookAt(xmf3Position, XMFLOAT3(0, 1.0, 0));
		}
