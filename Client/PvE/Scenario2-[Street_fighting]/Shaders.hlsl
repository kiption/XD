...
#define SKINNED_ANIMATION_BONES			128				// 스킨메쉬를 그리기 위한 뼈의 개수

cbuffer cbBoneOffsets : register(b7)
{
	float4x4 gpmtxBoneOffsets[SKINNED_ANIMATION_BONES];			// 본 오프셋 
};

cbuffer cbBoneTransforms : register(b8)
{
	float4x4 gpmtxBoneTransforms[SKINNED_ANIMATION_BONES];		// 각 뼈들의 변환행렬을 바꿔놓은것
};
...
struct VS_SKINNED_STANDARD_INPUT
{
	...
	float3 tangent : TANGENT;
	float3 bitangent : BITANGENT;
	uint4 indices : BONEINDEX;			// 뼈들의 인덱스
	float4 weights : BONEWEIGHT;		// 뼈들의 웨이트
};
...
VS_STANDARD_OUTPUT VSSkinnedAnimationStandard(VS_SKINNED_STANDARD_INPUT input)
{
	...
	{
		mtxVertexToBoneWorld = mul(gpmtxBoneOffsets[input.indices[i]], gpmtxBoneTransforms[input.indices[i]]);	// 뼈들의 정점 행렬
		output.positionW += input.weights[i] * mul(float4(input.position, 1.0f), mtxVertexToBoneWorld).xyz;		// 웨이트와 곱한 변환된 행렬(각 뼈들의 정점의 좌표)
