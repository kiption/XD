...
#else
	xmvPickRayDirection->x = xmvPickPosition.x * xmf4x4Inverse._11 + xmvPickPosition.y * xmf4x4Inverse._21 + xmvPickPosition.z * xmf4x4Inverse._31;
	xmvPickRayDirection->y = xmvPickPosition.x * xmf4x4Inverse._12 + xmvPickPosition.y * xmf4x4Inverse._22 + xmvPickPosition.z * xmf4x4Inverse._32;
	xmvPickRayDirection->z = xmvPickPosition.x * xmf4x4Inverse._13 + xmvPickPosition.y * xmf4x4Inverse._23 + xmvPickPosition.z * xmf4x4Inverse._33;
	xmvPickRayOrigin->x = xmf4x4Inverse._41;
	xmvPickRayOrigin->y = xmf4x4Inverse._42;
	xmvPickRayOrigin->z = xmf4x4Inverse._43;
	*xmvPickRayDirection = Vector3::Normalize(*xmvPickRayDirection);
#endif
}

int CGameObject::PickObjectByRayIntersection(XMFLOAT3 xmPickPosition, XMFLOAT4X4 xmmtxView, float* pfHitDistance)
{
	int nIntersected = 0;
	...
	{
		//모델 좌표계의 광선을 생성한다. 
		GenerateRayForPicking(xmPickPosition, &m_xmf4x4World, xmmtxView, &xmf3PickRayOrigin, &xmf3PickRayDirection);
...
CSkyBox::CSkyBox(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature) : CGameObject(1, 1)
{
	...
	pSkyBoxTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"SkyBox/Winter-sky.dds", RESOURCE_TEXTURE_CUBE, 0);
...
CHeightMapTerrain::CHeightMapTerrain(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, LPCTSTR pFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) : CGameObject(0, 1)
{
	...
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Terrain/Snowsurface.dds", RESOURCE_TEXTURE2D, 0);
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Terrain/Snowsurface.dds", RESOURCE_TEXTURE2D, 1);
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Terrain/Snowsurface.dds", RESOURCE_TEXTURE2D, 2);
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Terrain/Water_Base_Texture_0.dds", RESOURCE_TEXTURE2D, 3);
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Terrain/SnowGlass.dds", RESOURCE_TEXTURE2D, 4);
...
CTerrainWater::CTerrainWater(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, float nWidth, float nLength) :CGameObject(1, 1)
{
	...
	pWaterTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Terrain/Water_Base_Texture_0.dds", RESOURCE_TEXTURE2D, 0);
	pWaterTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Terrain/Water_Detail_Texture_0.dds", RESOURCE_TEXTURE2D, 1);
	pWaterTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Terrain/Water_Texture_Alpha.dds", RESOURCE_TEXTURE2D, 2);
...
CUseWaterMoveTerrain::CUseWaterMoveTerrain(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, LPCTSTR pFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) :CGameObject(0, 1)
{
	...
	pWaterMoveTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Terrain/Water_Base_Texture_0.dds", RESOURCE_TEXTURE2D, 0);
...
void CSnowObject::Animate(float fTimeElapsed)
{
	m_xmf4x4Transform._41 -= 2.0f;
