...
const wchar_t*CAnimationSet::GetCallbackData()
{
...
//			m_fPosition = fTrackPosition - int(fTrackPosition / m_fLength) * m_fLength;
	...
	{
		const wchar_t*pCallbackData = GetCallbackData();
...
void CAnimationSet::SetCallbackKey(int nKeyIndex, float fKeyTime, const wchar_t*pData)
{
...
void CAnimationSets::SetCallbackKey(int nAnimationSet, int nKeyIndex, float fKeyTime, const wchar_t*pData)
{
...
void CAnimationController::SetCallbackKey(int nSkinnedMesh, int nAnimationSet, int nKeyIndex, float fKeyTime, const wchar_t *pData)
{
...
CGameObject::CGameObject()
{
	...
	m_xmf4x4BaseWorld = Matrix4x4::Identity();
}
...
void CGameObject::UpdateTransform(XMFLOAT4X4 *pxmf4x4Parent)
{
	...
	m_xmf4x4BaseWorld = (pxmf4x4Parent) ? Matrix4x4::Multiply(m_xmf4x4Transform, *pxmf4x4Parent) : m_xmf4x4Transform;
...
{
	//CBV 
	//UINT ncbElementBytes = ((sizeof(CB_MODEL_INFO) + 255) & ~255); //256의 배수
...
void CGameObject::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	//memcpy(&m_pcbMappedGameObject->m_xmf3Color, &m_xmf3Color, sizeof(XMFLOAT3));
	...
	//pd3dCommandList->SetGraphicsRoot32BitConstants(15, 3, &m_xmf3Color, 16);
}
...
void CGameObject::SetScale(float x, float y, float z)
{
	...
	m_xmf4x4Transform = Matrix4x4::Multiply(mtxScale, m_xmf4x4BaseWorld);
...
CGameObject* CGameObject::LoadModelGeometryFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, const char* pstrFileName, CShader* pShader)
{
	return nullptr;
}
...
CHeightMapTerrain::CHeightMapTerrain(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, LPCTSTR pFileName, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) : CGameObject(1)
{
	...
	pTerrainBaseTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Terrain/Detail_Texture_6-1.dds", 0);

	CTexture *pTerrainDetailTexture = new CTexture(1, RESOURCE_TEXTURE2D, 0);
	pTerrainDetailTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Terrain/Asphelt.dds", 0);
...
CSkyBox::CSkyBox(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature) : CGameObject(1)
{
	...
	pSkyBoxTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"SkyBox/StarFiled.dds", 0);
