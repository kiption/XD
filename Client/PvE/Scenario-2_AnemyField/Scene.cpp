...
void CScene::LoadSceneObjectsFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, char* pstrFileName)
{
	...
	m_ppGameObject = new CGameObject * [m_nObjects];
	...
	for (int i = 0; i < m_nObjects; i++)
	{
		...
		for (int j = 0; j < i; j++)
		{
			if (!strcmp(pstrGameObjectName, m_ppGameObject[j]->m_pstrName))
			{
				pMesh = m_ppGameObject[j]->m_pMesh;
				break;
			}
		}
		if (!pMesh)
		...
		pGameObject->SetPosition(500.0,m_pTerrain->GetHeight(400.0,800.0)+1.0f, 900.0);
		pGameObject->SetScale(6.0,6.0,6.0);
		pGameObject->Rotate(0.0,180.0,0.0);
		m_ppGameObject[i] = pGameObject;
	}
...
void CScene::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	XMFLOAT3 xmf3Scale(8.0f, 4.0f, 9.0f);
	XMFLOAT4 xmf4Color(-0.001f, -0.001f, -0.001f, 1.0f);
	...
	CLoadedModelInfo* pMonsterModel = CGameObject::LoadGeometryAndAnimationFromFile(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, "Model/Angrybot.bin", NULL);
	...
	m_ppHierarchicalGameObjects[1]->Rotate(0.0, 90.0, 0.0);
	...
	m_ppHierarchicalGameObjects[2]->Rotate(0.0, 90.0, 0.0);
	...
	m_ppHierarchicalGameObjects[3]->Rotate(0.0, 90.0, 0.0);
...
void CScene::ReleaseObjects()
{
	...
	if (m_ppGameObject)
	{
		for (int j = 0; j < m_nObjects; j++) if (m_ppGameObject[j]) delete m_ppGameObject[j];
...
ID3D12RootSignature* CScene::CreateGraphicsRootSignature(ID3D12Device* pd3dDevice)
{
	...
	pd3dRootParameters[15].Constants.ShaderRegister = 9; //Model
...
void CScene::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	UINT ncbGElementBytes = ((sizeof(CB_MODEL_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbGameObject = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbGElementBytes, D3D12_HEAP_TYPE_UPLOAD,D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
...
void CScene::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	for (int i = 0; i < m_nObjects; i++)
	{
		...
		XMStoreFloat4x4(&xmf4x4world, XMMatrixTranspose(XMLoadFloat4x4(&m_ppGameObject[i]->m_xmf4x4World)));
		::memcpy(&m_pcbMappedGameObject->m_xmf4x4World, &xmf4x4world, sizeof(XMFLOAT4X4));
		::memcpy(&m_pcbMappedGameObject->m_xmf3Color, &m_ppGameObject[i]->m_xmf3Color, sizeof(XMFLOAT3));
	}
}
...
void CScene::ReleaseUploadBuffers()
{
	...
	if (m_ppGameObject)
	{
		for (int j = 0; j < m_nObjects; j++) if (m_ppGameObject[j]) m_ppGameObject[j]->ReleaseUploadBuffers();
	}
...
void CScene::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	D3D12_GPU_VIRTUAL_ADDRESS d3dcbModelGpuVirtualAddress = m_pd3dcbGameObject->GetGPUVirtualAddress();
	...
	for (int j = 0; j < m_nObjects; j++)
	{
		if (m_ppGameObject[j]) m_ppGameObject[j]->SceneModelRender(pd3dCommandList, pCamera);
	}
