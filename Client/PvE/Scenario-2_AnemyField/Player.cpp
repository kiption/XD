...
void CPlayer::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	if (nCameraMode == THIRD_PERSON_CAMERA) {
		...
		//CGameObject::SceneModelRender(pd3dCommandList, pCamera);
	}
}
...
CCamera *CTerrainPlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	switch (nNewCameraMode)
	{
		...
		case THIRD_PERSON_CAMERA:
			...
			m_pCamera = OnChangeCamera(THIRD_PERSON_CAMERA, nCurrentCameraMode);
			m_pCamera->SetTimeLag(0.25f);
			m_pCamera->SetOffset(XMFLOAT3(0.0f, -10.0f, -35.0f));
			m_pCamera->SetPosition(Vector3::Add(XMFLOAT3(this->GetPosition().x, this->GetPosition().y, this->GetPosition().z), m_pCamera->GetOffset()));
