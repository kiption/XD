...
{
	cout << "≈∞ ¥≠∏≤ ¿¸ Pitch Angle: " << ((HeliPlayer*)m_pPlayer)->m_fPitch << endl;
	cout << "≈∞ ¥≠∏≤ ¿¸ Roll Angle: " << ((HeliPlayer*)m_pPlayer)->m_fRoll << endl;
	...
	if (!bProcessedByScene)
	{
		...
		if (pKeysBuffer[VK_UP] & 0xF0) {
			...
			if (((HeliPlayer*)m_pPlayer)->m_fPitch < 15.0f) {
				...
				cout << "≈∞ ¥≠∏≤ »ƒ Pitch Angle: " << ((HeliPlayer*)m_pPlayer)->m_fPitch << endl;
			}
		...
		if (pKeysBuffer[VK_LEFT] & 0xF0) {
			...
			if (abs(((HeliPlayer*)m_pPlayer)->m_fRoll) < 45.0f) {
				...
				cout << "≈∞ ¥≠∏≤ »ƒ Roll Angle: " << ((HeliPlayer*)m_pPlayer)->m_fRoll << endl;
			}
		}
...
{
	UINT ncbElementBytes = ((sizeof(CB_FRAMEWORK_INFO) + 255) & ~255); //256Ïùò Î∞∞Ïàò
...
void CGameFramework::FrameAdvance()
{
	...
	if (UI_Switch) {
		if (m_nMode = SCENE1STAGE) {
			// ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ UI 
			...
			// ÔøΩÔøΩÔøΩÔøΩÔøΩ÷¥ÔøΩ NPC ÔøΩÔøΩÔøΩÔøΩ UI
			...
			// ÔøΩ—æÔøΩ ÔøΩÔøΩÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ UI
...
void CGameFramework::CreateDirect2DDevice()
{
	...
	hResult = m_pdWriteFactory->CreateTextFormat(L"ÌÖçÏä§Ìä∏ Î†àÏù¥ÏïÑÏõÉ", NULL, DWRITE_FONT_WEIGHT_DEMI_BOLD, DWRITE_FONT_STYLE_OBLIQUE, DWRITE_FONT_STRETCH_NORMAL, 24.0f, L"ko-kr", &m_pdwFont);
	...
	hResult = m_pdWriteFactory->CreateTextLayout(L"ÌÖçÏä§Ìä∏ Î†àÏù¥ÏïÑÏõÉ", 6, m_pdwFont, 1024, 1024, &m_pdwTextLayout);
...
//==================================================
//			  ÏÑúÎ≤Ñ ÌÜµÏã†Ïóê ÌïÑÏöîÌïú Ìï®ÏàòÎì§
//==================================================
...
void CGameFramework::setPosition_OtherPlayer(int id, XMFLOAT3 pos)
{
	...
	if (id < 0 || id > 5) return;	// Î∞∞Ïó¥ Î≤îÏúÑ Î≤óÏñ¥ÎÇòÎäî Í±∞ Î∞©ÏßÄ
