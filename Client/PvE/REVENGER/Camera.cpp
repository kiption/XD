...
void CSpaceShipCamera::Rotate(float x, float y, float z)
{
	...
	{
		m_fPitch += x;
		if (m_pPlayer->HeliPitch == true && m_pPlayer->SoldiarPitch == false)
		{
			if (m_fPitch > +20.0f) { x -= (m_fPitch - 20.0f); m_fPitch = +20.0f; }
			if (m_fPitch < -15.0f) { x -= (m_fPitch + 15.0f); m_fPitch = -15.0f; }
		}
		if (m_pPlayer->SoldiarPitch == true && m_pPlayer->HeliPitch == false)
		{
			if (m_fPitch > +45.0) { x -= (m_fPitch - 45.0); m_fPitch = +45.0; }
...
void CSpaceShipCamera::Update(XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	if (m_pPlayer)
	{
		...
		XMFLOAT3 xmf3Position = Vector3::Add(XMFLOAT3(m_pPlayer->m_xmf3Position), xmf3Offset);
...
void CThirdPersonCamera::SetLookAt(XMFLOAT3& xmf3LookAt)
{
	...
	m_xmf3Right = XMFLOAT3(mtxLookAt._11, mtxLookAt._21, mtxLookAt._31);
	m_xmf3Up = XMFLOAT3(0, 1, 0);
	m_xmf3Look = XMFLOAT3(mtxLookAt._13 * 4, mtxLookAt._23 / 4, mtxLookAt._33 * 4);
}
