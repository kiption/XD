...
{
	CSkyBoxMesh* pSkyBoxMesh = new CSkyBoxMesh(pd3dDevice, pd3dCommandList, 30.0f, 30.0f, 10.0f);
...
CHeightMapTerrain::CHeightMapTerrain(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, LPCTSTR pFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) : CGameObject(0, 1)
{
	...
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Glacier.dds", RESOURCE_TEXTURE2D, 1);
	...
	pTerrainTexture->LoadTextureFromDDSFile(pd3dDevice, pd3dCommandList, L"Image/Ice_Detail.dds", RESOURCE_TEXTURE2D, 4);
	//SnowMountain_Detail
...
void CExplosionObject::Animate(float fTimeElapsed)
{
	...
	if (m_nMaterials > 1)
	{
		for (int i = 0; i < m_nMaterials; i++)
		{
			if (m_ppMaterials[i])
			{
				if (m_ppMaterials[i]->m_pTexture) m_ppMaterials[0]->m_pTexture->AnimateRowColumn(m_fTime);
			}
		}
	}
...
CUseWaterMoveTerrain::CUseWaterMoveTerrain(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, LPCTSTR pFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) :CGameObject(0, 1)
{
	...
	pWaterMoveShader->CreateShaderResourceViews(pd3dDevice, pWaterMoveTexture, 0, 11);
...
void CCrossHeadObject::Animate(float fTimeElapsed)
{
}
